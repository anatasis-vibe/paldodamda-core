...
Design rules:
  ...
  - failed_rows = actual row-level errors (exception during parse/normalize/match).
...
class ImportStats:
    total_rows:     int = 0
    success_rows:   int = 0
    failed_rows:    int = 0   # actual errors (exceptions) — NOT review queue
...
class ImportEngine:
                    ...
                    if match.matched:
                        ...
                        if price_changed:
                            self._record_price_history(
                                conn, offer_id, supplier_id, match.product_id,
                                normalized.price, import_id
                            )
                        ...
                        self.log.debug(
                            f"  [REVIEW] {parsed.original_name!r} -> product_name={normalized.product_name!r}"
                        )

                except Exception as row_err:
                    stats.failed_rows += 1
                    self.log.warning(f"  [ROW ERROR] {parsed.original_name!r}: {row_err}")
    ...
    def _create_import_history(self, conn: sqlite3.Connection, supplier_id: int, file_name: str) -> int:
        cur = conn.execute(
    ...
    def _update_import_history(self, conn: sqlite3.Connection, import_id: int, stats: ImportStats) -> None:
        conn.execute(
            """UPDATE import_history
               SET total_rows=?, success_rows=?, failed_rows=?, new_products=?, updated_prices=?
               WHERE id=?""",
    ...
    def _save_offer(
        ...
        normalized: NormalizedRow, match: MatchResult,
        supplier_id: int, import_id: int,
    ) -> tuple[int, bool]:
        """Insert product_offer row. Returns (offer_id, price_changed)."""
    ...
    def _record_price_history(
        self, conn: sqlite3.Connection,
        offer_id: int, supplier_id: int, product_id: Optional[int],
        price: Optional[int], import_id: int,
    ) -> None:
        conn.execute(
            "INSERT INTO price_history (offer_id, supplier_id, product_id, price, import_id) VALUES (?,?,?,?,?)",
            (offer_id, supplier_id, product_id, price, import_id),
        )

    def _save_review_queue(
        self, conn: sqlite3.Connection,
        parsed, normalized: NormalizedRow,
        supplier_id: int, import_id: int,
    ) -> None:
        attrs = json.dumps({
        ...
        conn.execute(
            """INSERT INTO review_queue
               (import_id, supplier_id, supplier_name, original_name,
                normalized_attrs, price, file_name)
               VALUES (?,?,?,?,?,?,?)""",
            (
